...
const GetExists = () => {
  ...
  const [tokenId, setTokenId] = useState('');
	...
	const fetchData = () => {
	    ...
	    resolve(erc1155.getExists!(tokenId!));
	  });
	};
	...
	const { data, status, isFetching, error, refetch } = useQuery("fetchGetExists", fetchData, {
	...
	return (
		<Box>
			<h4>Check If Token Identifier Exists</h4>
			<p>Checks if a given token identifier has at least 1 token in circulation</p>
						...
						<Content>
							...
							<Button onClick={handleClick}>
								{!account
									...
									: !hidden
									? 'Is Approved to Send'
									: data ? data.toString() : 'Token Id Has Not Been Minted'}
							</Button>
						</Content>
					</Parameters>
				</Item>
